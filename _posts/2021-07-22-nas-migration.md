...
* A NAS, making my HDS redundant using RAID, pooling these HDs, presenting them to me as one big
disk; And sharing my files to other devices through SAMBA.
* A entertainment server, serving music, movies and series to my other devices using Plex.
...
application data, backups,etc. I have no words for ZFS, if you don't know, I suggest that you look
into it, it's one of the most incredibly pieces of technology that I have worked it, and I think no
other file system is close to achieve what ZFS does.

* **Snapraid**: 4 4TBs HDs in "RAID5. This software is AWESOME, it acts like a RAID system, but on a
file system level, that way all the hard drives are formatted normally in ext, ntfs, xfs, whatever
you want, and then you assign one or two hard drives to store parity data (acting like a RAID5
...
single directory with all the files to read and write to, and it will distribute all the data that
you write to the drives, following the rule of your choice.
...
## Virtual Machines
Open Media Vault has a nice Virtualbox plugin, I used it to host my Home Assistant, which I like to
keep on its on OS along with mqtt and network stuff, this way I can migrate the whole machine to
another host easily and keep my home automations working.

This is also where I put my development machines, this way I have access anywhere to work, and keep
host clean.

# What is wrong ?
Fortunately, not much. But some things made me move on.
* Most of the features that I used was through community maintained plugins.
...
the server, I like to tinker ON my server, and have the minimum amount of maintenance possible.
...
This way, I settled with ubuntu server 20.04. Although I like OSes
on the Red Hat family, you can't go wrong the Debian way too, and ZFS being a first class citizen
...
container management like portainer, but it's way much.
Then, I found Glances. It runs inside a container, and just give me a nice screen with all the info
I need, and on top of that, it has a really nice integration with home assistant.
...
## Virtual Machines
I wanted to drop VirtualBox, although in the end I was always able to do what I want, managing my VMS,
...
in a machine separate from the containers, move the home assistant to a NUC, so many cool things to
do.
